    ...
    async def _process_event(self, event_id: str, fields: dict[str, str]) -> None:
        try:
            ...
            output_payload, generation_ms, model_name = await generate_structured_design(
                input_payload, scale_stance=scale_stance
            )
            ...
            try:
                ...
                await self.redis.xadd(
                    ...
                    {
                        "type": "design.updated",
                        "payload_json": json.dumps({"design_id": design_id, "status": "completed"})
                    },
                ...
                if design_id > 0:
                    with SessionLocal() as db:
                        ...
                        if design and design.status == "generating":
                            ...
                            await self.redis.xadd(
                                stream_route,
                                {"type": "design.updated", "payload_json": json.dumps({"design_id": design_id, "status": "failed"})},
                                maxlen=self.settings.stream_maxlen_approx,
                                approximate=True,
                            )
            except Exception:
