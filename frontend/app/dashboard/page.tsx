...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    api<OpsSummary>("/dashboard/ops-summary")
      .then(setOps)
      .catch(() => setOps(null));
  }, [q]);
  ...
  const stats = [
    {
      ...
      value: String(ops?.total_designs ?? items.length),
      icon: Activity,
    },
    {
      label: t("dashboard.reviewPending"),
      value: String(ops?.review_pending_count ?? 0),
      icon: Clock3,
    },
    {
      label: t("dashboard.approvedDesigns"),
      value: String(ops?.approved_count ?? 0),
      icon: ShieldCheck,
    },
    {
      label: t("dashboard.monthlyCostRange"),
      value: ops
        ? `$${ops.monthly_cost_min_total.toLocaleString()} - $${ops.monthly_cost_max_total.toLocaleString()}`
        : "—",
      icon: DollarSign,
    },
    {
      label: t("dashboard.riskDrift"),
      value: String(ops?.risk_drift_count ?? 0),
      icon: AlertTriangle,
    },
  ];
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-[0.98] duration-500 pb-20">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
        {stats.map((item) => (
          <Card
            key={item.label}
            className="group relative overflow-hidden rounded-2xl border border-white/5 bg-[#0a0a0a] p-6 transition-all hover:bg-white/[0.02] hover:border-white/10 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.5)]"
          >
      ...
      <section className="space-y-4 mt-6">
        <div className="flex items-center justify-between pb-2">
          <h2 className="text-base font-medium tracking-wide text-white/90">
        ...
        {error ? (
          <Card className="rounded-2xl border-red-500/10 bg-red-500/[0.02] p-8 text-center">
            ...
            <Button
              className="mt-4 bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/20"
              variant="outline"
              ...
              <Link key={d.id} href={`/dashboard/designs/${d.id}`}>
                <Card className="group flex flex-col gap-3 rounded-2xl border border-white/5 bg-[#0a0a0a] p-6 sm:flex-row sm:items-center sm:justify-between transition-all duration-300 hover:bg-white/[0.03] hover:border-white/10 hover:shadow-[0_0_30px_-5px_rgba(255,255,255,0.03)] cursor-pointer">
                  <div>
                    ...
                    <div className="mt-1.5 text-[13px] text-white/40 font-light">
                      {d.project_type}
                    </div>
                  </div>
                  <div className="flex items-center gap-6 justify-between sm:justify-end">
                    ...
                    <div className="flex h-8 w-8 items-center justify-center rounded-full border border-white/5 bg-transparent transition-colors group-hover:bg-white/10 group-hover:border-white/20">
                      <span className="text-white/40 group-hover:text-white opacity-0 -translate-x-2 transition-all group-hover:opacity-100 group-hover:translate-x-0">
                        →
                      </span>
                    </div>
                  </div>
